...
import { SALES_ORDER_DETAIL } from '@/router/config/order-manage/path'
...
function renderBuylerInfo(value: string, record: Order): React.ReactNode {
  return (
    <>
      <p className={classnames(styles.sellerName, styles.oneRowEllipse)}>{record.buyerName}</p>
      <p className={styles.sellerType}>{record.buyerType}</p>
...
const columns = [
  {
    ...
    render: (value: string, record: Order) => (
      <Link to={{ pathname: SALES_ORDER_DETAIL, search: `?orderId=${value}` }}>{record.orderId}</Link>
    )
  },
  {
    key: 'goodsInfo',
