...
import { namespace, IState } from '@/models/order-manage/purchase/detail'
...
const OrderManageDetail: React.FC<IProps> = props => {
  ...
  const sellerInfo = customerInfo.sellerInfo || {}
      ...
      <Tabs tabBarGutter={33} tabBarStyle={{ paddingLeft: '25px' }} defaultActiveKey="1">
        <TabPane tab="订单信息" key="1">
          <section className={styles.sectionBody}>
            ...
            <Card title="供应商信息">
              <Descriptions column={2}>
                <Item label="商家名称">{sellerInfo.memberName}</Item>
                <Item label="商家类型">{sellerInfo.memberTypeDesc}</Item>
                <Item label="联系方式">{sellerInfo.mobile}</Item>
                <Item label="商家编号">{sellerInfo.id}</Item>
              </Descriptions>
              <DescItem label="联系地址">
                {sellerInfo.provinceName} {sellerInfo.cityName}
                {sellerInfo.districtName}
                {sellerInfo.address}
              </DescItem>
            </Card>
            <SingerLineCards
